  ...
  useEffect(() => {
    if (!isHovered) {
      const interval = setInterval(() => {
        ...
        setCurrentItem((currentItem) => (currentItem === 4 ? 0 : currentItem + 1));
      }, 3000);
    ...
    >
      ...
      <div className="relative w-full h-full">
            ...
            return (
              ...
              >
                <div className="bg-white dark:bg-slate-800 relative w-[15rem] lg:w-[30rem] h-full mx-auto overflow-hidden rounded-lg transform-style-preserve-3d flex justify-center">
                  <div
                    className={\`w-[30rem] h-full duration-300 $\{
                      index === currentItem
                        ? 'opacity-100'
                        : index === currentItem + 1 || index === currentItem - 1
                        ? 'opacity-70'
                        : index === currentItem - (items.length - 1) ||
                          index === currentItem + (items.length - 1)
                        ? 'opacity-70'
                        : 'opacity-20'
                    }\`}
        ...
        <button
          onClick={() => {
            rotate(1);
            setCurrentItem((currentItem) => (currentItem === 0 ? items.length - 1 : currentItem - 1));
          }}
        ...
        <button
          onClick={() => {
            rotate(-1);
            setCurrentItem((currentItem) => (currentItem === items.length - 1 ? 0 : currentItem + 1));
          }}

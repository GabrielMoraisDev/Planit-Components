...
export default function Blog() {
  ...
  return (
    <div className='bg-slate-200 lg:bg-slate-200 mt-16'>
      <div className="w-full h-[22rem] bg-slate-800 bg-center bg-cover flex justify-start place-items-center text-slate-900" 
      style={{backgroundImage: `url('https://img.freepik.com/vetores-gratis/fundo-de-paisagem-de-verao-para-zoom_52683-42162.jpg?t=st=1739411112~exp=1739414712~hmac=ab98c321de7cc4e633095f87b49eb7886f6e2f3e0aa1677070c9e4b0034c6036&w=1380')`,
      backgroundColor: "rgba(255, 255, 255, 0.5)",
      backgroundBlendMode: "overlay",}}>
        <div className="inline mt-[-8rem] ml-5 md:ml-12">
          <h1 className='text-3xl font-extrabold'>Title of post here!</h1>
          <p className='text-xl font-bold'>And subtitle, as well</p>
          <p className='text-base font-bold w-full md:w-[60%] mt-5'>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Aperiam commodi possimus consectetur animi reiciendis dolorum.</p>
        </div>
      </div>
      <div className="bg-gradient-to-b from-transparent via-transparent to-slate-200 w-full h-72 mt-[-18rem]"></div>
      <div className="w-[97%] h-auto bg-white m-auto mt-[-8rem] rounded-md py-5 px-4 md:px-8 flex gap-5">
        <div className="lg:w-[70%] text-base h-auto text-slate-800">
          ...
          <div className="inline sm:flex gap-4">

            <div className="my-3 w-full sm:w-[50%] h-auto bg-white p-3 rounded-md border-slate-300 hover:border-slate-400 cursor-pointer border flex place-items-center justify-center duration-300">
          ...
          <div style={{backgroundImage: `url("https://img.freepik.com/vetores-gratis/fundo-de-paisagem-de-verao-para-zoom_52683-42162.jpg?t=st=1739411112~exp=1739414712~hmac=ab98c321de7cc4e633095f87b49eb7886f6e2f3e0aa1677070c9e4b0034c6036&w=1380")`}} className='w-full h-72 rounded-md my-5 bg-cover bg-center'></div>
        ...
        <div className="w-full bg-white rounded-t-lg py-3 grid grid-cols-1 lg:grid-cols-3 justify-center gap-4">
          {cardData.map((card, index) => (
            <div key={index} className='w-full h-52 lg:h-52 xl:h-52 border border-slate-300 rounded-lg relative cursor-pointer overflow-hidden duration-300 bg-white hover:bg-slate-200 m-0' onMouseEnter={()=>{setHovered(index);hoveredCard(index)}} onMouseLeave={()=>{setHovered(-1);hoveredClear()}}>
              <div className={`w-full ${hovered === index?'h-full bg-right':'h-full bg-left'} duration-1000 bg-cover absolute top-0 left-0 z-10 bg-slate-200`}>
                <div className={`w-full h-full flex justify-center place-items-center ${hovered === index?'bg-white/60 text-slate-300':'bg-trasnparent text-slate-500'} duration-300`}>
                  <Icon.FileEarmarkImage className='text-6xl mt-[-2.3rem]'/>
                </div>
                  <h1 className={`truncate lg:text-lg absolute bottom-0 bg-white w-full text-slate-800 py-2 left-0 text-xl font-bold ${hovered === index?'opacity-0 left-0 duration-300':'left-0 px-4 opacity-700 duration-300'} z-20`}>{card.title}</h1>
                  <p className={`text-center w-[80%] truncate text-2xl font-bold absolute top-6 text-slate-800 ${hovered4 === index?'opacity-100 right-6 lg:right-5 xl:right-9':'opacity-0 right-0'} duration-300`}>{card.title}</p>
                  <p className={`text-sm lg:text-sm xl:text-base text-center w-[80%] font-bold absolute top-16 text-slate-800 ${hovered3 === index?'opacity-100 right-6 lg:right-5 xl:right-9':'opacity-0 right-[-0.5rem]'} duration-300`}>{truncateDescription(card.description, 80)}</p>
                  <p className={`underline text-center w-[80%] text-sm font-bold absolute bottom-10 text-slate-800 ${hovered2 === index?'opacity-100 right-6 lg:right-5 xl:right-9':'opacity-0 right-[-0.5rem]'} duration-300`}>See More</p>
                  <p className={`text-center w-[80%] flex justify-center place-items-center text-sm font-bold absolute bottom-5 text-slate-800 ${hovered1 === index?'opacity-100 right-6 lg:right-5 xl:right-9':'opacity-0 right-[-0.5rem]'} duration-300`}><Icon.Calendar className='mr-1'/>01/01/2025</p>
                </div>
            </div>
            ))}
          </div>
    ...
    {/* Only Desktop */}
        <div className="w-[30%] sticky top-20 h-1 hidden lg:inline">
          <h1 className='ml-3 font-bold text-xl mb-3 text-slate-800'>Latest News</h1>
          ...
          <div className="ml-2">
            ...
            <h1 className='font-bold text-lg mt-3 w-full truncate text-slate-800'>Another title?</h1>
            <p className='max-w-full truncate text-slate-600'>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Unde optio repellat, modi, consequatur, consectetur eveniet nisi ex quos eaque odio sit in et veniam natus nesciunt minima consequuntur. Ut, obcaecati?</p>
            <a href='#' className="text-sm underline rounded-md text-slate-500">See More</a>
          </div>
